...
local function StartGlow(frame, glowType, glowSettings)
    ...
    SL.HideAllGlows(frame)  -- Stop any existing glow first

    local s = glowSettings or {}
    if glowType == "pixel" then
        SL.ShowPixelGlow(frame, s.color, math.floor(s.lines or 8), s.frequency, nil, s.thickness, s.xOffset, s.yOffset)
    elseif glowType == "autocast" then
        SL.ShowAutocastGlow(frame, s.color, math.floor(s.lines or 8), s.frequency, s.thickness, s.xOffset, s.yOffset)
    elseif glowType == "button" then
        SL.ShowButtonGlow(frame, s.color, s.frequency)
    elseif glowType == "proc" then
        -- proc glow: LCG direct (SL에서 미래 지원 시 전환)
        local LCG = LibStub("LibCustomGlow-1.0", true)
        if LCG and LCG.ProcGlow_Start then
            LCG.ProcGlow_Start(frame, {color = s.color or {0.95, 0.95, 0.32, 1}, duration = s.frequency or 1, startAnim = true})
        end
    end
end
...
local function StopAllGlows(frame)
    if not frame then return end
    SL.HideAllGlows(frame)
    -- ProcGlow는 SL에 없으므로 직접 stop
    local LCG = LibStub("LibCustomGlow-1.0", true)
...
-- Legacy fallback observation helpers live in TrackedAuraLegacyFallbackDriver.lua.
...
local cdmVisibility = {
    ...
    hiddenSpellIDs = setmetatable({}, { __mode = "k" }),
...
local function SetCachedCDMFrameHidden(frame, cooldownID, shouldHide, spellID)
    ...
    if shouldHide then
        ...
        cdmVisibility.hiddenSpellIDs[frame] = spellID
    ...
    elseif hiddenID or frame._ddingHidden then
        ...
        cdmVisibility.hiddenSpellIDs[frame] = nil
...
local function BuildCDMVisibilitySignature(trackers)
    ...
    wipe(cdmVisibility.desiredSpellIDs)
    local ids = {}
    for _, tracker in ipairs(trackers or {}) do
        local settings = type(tracker) == "table" and tracker.settings
        if settings and settings.hideFromCDM then
            local spellID = tonumber(tracker.spellID)
            if CDMCompat and CDMCompat:IsUsableID(spellID)
                and not cdmVisibility.desiredSpellIDs[spellID]
            then
                cdmVisibility.desiredSpellIDs[spellID] = true
                ids[#ids + 1] = "s:" .. spellID
            end

            local cooldownID = ResolveTrackedAuraCooldownID(tracker)
            if cooldownID and not cdmVisibility.desired[cooldownID] then
                cdmVisibility.desired[cooldownID] = true
                ids[#ids + 1] = "c:" .. cooldownID
            end
        end
    end
    table.sort(ids)
...
local function RefreshCDMVisibilityCache(trackers)
    ...
    for frame, hiddenID in pairs(cdmVisibility.hiddenFrames) do
        local spellID = cdmVisibility.hiddenSpellIDs[frame]
        if not cdmVisibility.desired[hiddenID]
            and not cdmVisibility.desiredSpellIDs[spellID]
        then
    ...
    local function CacheFrame(frame, forcedCooldownID)
        ...
        local spellID = CDMCompat:ResolveFrameSpellID(frame)
        local shouldHide = cdmVisibility.desired[cooldownID] == true
            or cdmVisibility.desiredSpellIDs[spellID] == true
        SetCachedCDMFrameHidden(frame, cooldownID, shouldHide, spellID)
    end
    ...
    for cooldownID in pairs(cdmVisibility.desired) do
        ...
        if entry and not IsSecretValue(entry) then
            local isAuraEntry = entry.isAura == true or entry.viewerType == "aura"
            if isAuraEntry then
...
function ResourceBars:SetTrackedBuffHiddenInCDM(cooldownID, shouldHide, knownFrame)
    if not CDMCompat then return knownFrame end
    if CDMCompat:IsSettingsOpen() then
        return knownFrame
    end

    if not CDMCompat:IsUsableID(cooldownID) and knownFrame then
        cooldownID = CDMCompat:GetFrameCooldownID(knownFrame)
    end
    if not CDMCompat:IsUsableID(cooldownID) then return knownFrame end
    ...
    elseif knownFrame then
        local spellID = CDMCompat:ResolveFrameSpellID(knownFrame)
        SetCachedCDMFrameHidden(
            knownFrame,
            cooldownID,
            cdmVisibility.desired[cooldownID] == true
                or cdmVisibility.desiredSpellIDs[spellID] == true,
...
buffTrackerEventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
        ...
        if BUFF_TRACKER_DEBUG then
            print(string.format("[ManualTrack] SPELLCAST spellID=%s, trackedBuffs=%s, inCombat=%s",
                tostring(spellID), tostring(#trackedBuffs), tostring(inCombat)))
        end

        for barIndex, buff in ipairs(trackedBuffs) do
            if BUFF_TRACKER_DEBUG then
                print(string.format("[ManualTrack] Bar %s: trackingMode=%s, hasSettings=%s",
                    tostring(barIndex), tostring(buff.trackingMode), tostring(buff.settings ~= nil)))
            end
            ...
            if buff.trackingMode == "manual" and buff.settings then
                ...
                if BUFF_TRACKER_DEBUG then
                    ...
                    print(string.format("[ManualTrack] Bar %s: generators=%s, spellID=%s, inCombat=%s",
                        tostring(barIndex), tostring(genCount), tostring(spellID), tostring(inCombat)))
                end
                ...
                if genData then
                    if BUFF_TRACKER_DEBUG then
                        print(string.format("[ManualTrack] Bar %s: GENERATOR FOUND! stacks=%s, duration=%s",
                            tostring(barIndex), tostring(genData.stacks), tostring(genData.duration)))
                    end
                    if inCombat then
...
function ResourceBars:GetBuffTrackerBar()
    ...
    bar:SetPoint(initSelfPoint, anchor, anchorPoint, DDingUI:Scale(cfg.offsetX or 0), DDingUI:Scale(cfg.offsetY or 18))
...
SlashCmdList["BTDEBUG"] = function()
    ...
    print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: Buff Tracker Debug: " .. (BUFF_TRACKER_DEBUG and "ON" or "OFF")) -- [STYLE]
    ...
    for i, buff in ipairs(trackedBuffs) do
        ...
        print(string.format("  [%s] %s (cdID: %s, type: %s, mode: %s)", tostring(i), name, tostring(cdID), displayType, trackingMode))
        if buff.settings then
            local s = buff.settings
            print(string.format("      maxStacks=%s, hideWhenZero=%s, stackDuration=%s",
                tostring(s.maxStacks or 1), tostring(s.hideWhenZero), tostring(s.stackDuration or 30)))
            -- Show generators
            if s.generators and #s.generators > 0 then
                ...
                for _, gen in ipairs(s.generators) do
                    ...
                    print(string.format("        - %s (%s): stacks=%s, duration=%s",
                        tostring(gen.spellID), genName, tostring(gen.stacks), tostring(gen.duration)))
                end
            else
            ...
            if s.spenders and #s.spenders > 0 then
                ...
                for _, spend in ipairs(s.spenders) do
                    ...
                    print(string.format("        - %s (%s): consume=%s",
                        tostring(spend.spellID), spendName, tostring(spend.consume)))
                end
            end
            -- Show manual stacks for this buff
            local stacks, expiresAt = GetManualStacks(i)
            print(string.format("      manualStacks=%s, expiresAt=%s", tostring(stacks), tostring(expiresAt)))
        else
            print("      settings: (nil)")
        end
    end
    ...
    for spellID, stacks in pairs(generators) do
        ...
        print(string.format("  [%s] %s -> +%s stacks", tostring(spellID), name, tostring(stacks)))
        genCount = genCount + 1
    end
    if genCount == 0 then
    ...
    for spellID, consume in pairs(spenders) do
        local name = C_Spell.GetSpellName(spellID) or "?"
        print(string.format("  [%s] %s -> -%s stacks", tostring(spellID), name, tostring(consume)))
    ...
    for barIndex, bar in pairs(barFrames) do
        ...
        print(string.format("  [Bar %s] %s (type: %s)", tostring(barIndex), buffName, displayType))
    ...
    for barIndex, icon in pairs(iconFrames) do
        ...
        local buffInfo = trackedBuffs[barIndex]
        local buffName = buffInfo and buffInfo.name or "Unknown"
        print(string.format("  [Icon %s] %s", tostring(barIndex), buffName))
    ...
    if specCfg and specCfg.requireTalentID and specCfg.requireTalentID > 0 then
        ...
        print(string.format("  [%s] %s - %s", tostring(specCfg.requireTalentID), talentName, talentKnown and "|cff00ff00KNOWN|r" or "|cffff0000NOT KNOWN|r"))
    end
...
SlashCmdList["BTTEST"] = function()
    ...
    if testMode then
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cff00ff00Buff Tracker Test Mode: ON|r (ticker paused)") -- [STYLE]
...
GetTrackedBuffs = function()
    ...
    -- 기존 "기존 데이터 보호" 로직이 구 프로필의 추적 버프를 무시하던 버그 수정
    ...
    if rootCfg and rootCfg.trackedBuffsPerSpec then
        for oldSpecID, oldData in pairs(rootCfg.trackedBuffsPerSpec) do
            if type(oldData) == "table" and #oldData > 0 then
                ...
                -- 기존 글로벌 데이터와 병합 (cooldownID + displayType 기반 중복 체크)
        ...
        -- 마이그레이션 완료 후 프로필에서 제거
        rootCfg.trackedBuffsPerSpec = nil
    end

    -- [FIX] Legacy migration: 레거시 trackedBuffs → global (병합 방식)
...
local function CreateTrackedBuffBar(barIndex)
    ...
    bar.DonutCenter:SetColorTexture(0, 0, 0, 1)  -- 단색 검정 텍스쳐
...
local function CreateTrackedBuffIcon(barIndex)
    ...
    icon.DurationText = icon:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    ...
    icon.DurationText:SetShadowOffset(0, 0)
...
-- Play sound using LibSharedMedia or custom file path
local function PlayTrackerSound(soundKey, channel, customPath)
    -- Custom path takes priority if valid
...
local function CreateTrackedBuffText(barIndex)
    ...
    frame.DurationText:SetPoint("LEFT", frame.Text, "RIGHT", 4, 0)
    frame.DurationText:SetJustifyH("LEFT")
    frame.DurationText:SetFont(STANDARD_TEXT_FONT, 10, "OUTLINE")
    frame.DurationText:SetShadowOffset(0, 0)
...
function ResourceBars:UpdateSingleTrackedBuffBar(barIndex, trackedBuff, globalCfg)
    ...
    local durationTextY = settings.durationTextY or 0
    ...
    then
        auraStyle = {
            ...
            showTicks = showTicks,
            tickWidth = math.max(1, DDingUI:Scale(tickWidth)),
            tickPositions = durationTickPositions,
    ...
    if BUFF_TRACKER_DEBUG then
        print(string.format("[BuffTracker] Bar %s (%s): stacks=%s, hasData=%s, visible=%s, fillMode=%s, trackingMode=%s, isManual=%s",
            tostring(barIndex), trackedBuff.name or "?",
...
function ResourceBars:UpdateSingleTrackedBuffRing(barIndex, trackedBuff, globalCfg)
    ...
    if settings.ringOffsetX ~= nil and settings.ringOffsetY ~= nil then
        ...
        -- [12.0.1] growthSpacing만 사용 (ringSize 포함 X → 크기 변경 시 위치 이동 방지)
        local scaledOffsetX = DDingUI:Scale(ringOffsetX)
        local scaledOffsetY = DDingUI:Scale(ringOffsetY)
        local spacing = DDingUI:Scale(growthSpacing)
        local stackIndex = barIndex - 1
    ...
    -- [12.0.1] 앵커: ClearAllPoints → SetPoint("CENTER") → SetSize 순서
    ...
    -- ============================================================
    -- DURATION TEXT (bar/icon/text 모드와 동일한 별도 DurationText FontString) -- [12.0.1]
    -- ============================================================
    local showDurationText = settings.showDurationText or false
    if showDurationText and bar.DurationText then
        -- Settings
        local dtFont = settings.durationTextFont
        local dtSize = settings.durationTextSize or 10
        local dtAlign = settings.durationTextAlign or "CENTER"
        local dtX = settings.durationTextX or 0
        local dtY = settings.durationTextY or 0
        local dtColor = settings.durationTextColor or { 1, 1, 1, 1 }
        local dtDecimals = settings.durationDecimals or 1
        local dtWarning = settings.durationWarningEnabled or false
        local dtWarningThreshold = settings.durationWarningThreshold or 5
        local dtWarningColor = settings.durationWarningColor or { 1, 0.2, 0.2, 1 }

        -- Font
        local fontPath = dtFont and LSM:Fetch("font", dtFont) or STANDARD_TEXT_FONT
        bar.DurationText:SetFont(fontPath, dtSize, "OUTLINE")
        bar.DurationText:SetJustifyH(dtAlign)

        -- Position (링 기준 오프셋)
        bar.DurationText:ClearAllPoints()
        bar.DurationText:SetPoint("CENTER", bar, "CENTER", dtX, dtY)

        -- Color
        bar.DurationText:SetTextColor(dtColor[1] or 1, dtColor[2] or 1, dtColor[3] or 1, dtColor[4] or 1)
        ...
        if hasData and not isManualMode then
                    ...
                    pcall(function()
                        if HasAuraInstanceID(data.auraID) then
                            ...
                            if LegacyDurationDriver.HasRemainingDurationValue(remaining) then
                                ...
                                -- Warning color
                                if data.warningEnabled then
                                    local ok, isLow = pcall(function() return remaining <= data.warningThreshold end)
                                    if ok and isLow then
                                        local wc = data.warningColor
                                        bar.DurationText:SetTextColor(wc[1] or 1, wc[2] or 0.2, wc[3] or 0.2, wc[4] or 1)
                                    else
                                        local nc = data.normalColor
                        ...
                        elseif data.expiresAt then
                            local remaining = math_max(0, data.expiresAt - GetTime())
                            bar.DurationText:SetText(FormatDuration(remaining, data.durationDecimals))
                            if data.warningEnabled then
                                if remaining <= data.warningThreshold then
                                    local wc = data.warningColor
            ...
            bar.TextFrame._dtData = {
                ...
                auraID = auraInstanceID,
                expiresAt = expiresAt,
                durationDecimals = dtDecimals,
                warningEnabled = dtWarning,
                warningThreshold = dtWarningThreshold,
                warningColor = dtWarningColor,
                normalColor = dtColor,
            }
            -- 초기값
            if remainingDuration and remainingDuration > 0 then
                bar.DurationText:SetText(FormatDuration(remainingDuration, dtDecimals))
            end
            bar.DurationText:Show()
        ...
        elseif hasData and isManualMode then
            ...
            if not bar.TextFrame._hasDurationUpdate then
                RegisterDurationUpdate(bar.TextFrame, function(self, elapsed)
                    ...
                    local data = self._dtData
                    if not data or not data.showDurationText or not data.isManualMode then return end

                    local _, manualExp = GetManualStacks(data.barIndex)
                    if manualExp and manualExp > 0 then
                        local remaining = math_max(0, manualExp - GetTime())
                        bar.DurationText:SetText(FormatDuration(remaining, data.durationDecimals))
                        if data.warningEnabled then
                            if remaining <= data.warningThreshold then
                                local wc = data.warningColor
        ...
        elseif isInPreviewMode or isInMoverMode then
            -- 미리보기/이동 모드: 샘플 텍스트
            bar.DurationText:SetText(FormatDuration(18.5, dtDecimals))
            bar.DurationText:Show()
        else
...
function ResourceBars:UpdateSingleTrackedBuffIcon(barIndex, trackedBuff, globalCfg)
    ...
    if showDurationText and icon.DurationText then
        local dtFont = settings.durationTextFont
        local dtFontPath = dtFont and LSM:Fetch("font", dtFont) or STANDARD_TEXT_FONT
        local dtSize = settings.durationTextSize or 10
        local dtAlign = settings.durationTextAlign or "CENTER"
        local dtX = settings.durationTextX or 0
        local dtY = settings.durationTextY or 0
        ...
        icon.DurationText:SetPoint(dtAlign, icon, dtAlign, DDingUI:Scale(dtX), DDingUI:Scale(dtY))
        icon.DurationText:SetJustifyH(dtAlign)
        icon.DurationText:SetFont(dtFontPath, DDingUI:Scale(dtSize), "OUTLINE")
        icon.DurationText:SetTextColor(dtColor[1] or 1, dtColor[2] or 1, dtColor[3] or 1, dtColor[4] or 1)
            ...
            if not icon._hasDurationUpdate then
                RegisterDurationUpdate(icon, function(self, elapsed)
                    ...
                    if data.isManualMode then
                        ...
                        if expiresAt then
                            local remaining = math_max(0, expiresAt - GetTime())
                            icon.DurationText:SetText(FormatDuration(remaining, data.durationDecimals))
                            if data.warningEnabled then
                                if remaining <= data.warningThreshold then
                                    local wc = data.warningColor
                                    icon.DurationText:SetTextColor(wc[1] or 1, wc[2] or 0.2, wc[3] or 0.2, wc[4] or 1)
                                else
                                    local nc = data.normalColor
                                    icon.DurationText:SetTextColor(nc[1] or 1, nc[2] or 1, nc[3] or 1, nc[4] or 1)
                                end
                            end
                        end
                        return
        ...
        elseif isInPreviewMode or isInMoverMode then
            -- 미리보기/이동 모드: 샘플 텍스트
            icon.DurationText:SetText(FormatDuration(18.5, dtDecimals))
            icon.DurationText:Show()
        else
            -- 버프 없음: 숨기기 + OnUpdate 제거
            icon.DurationText:SetText("")
    ...
    if BUFF_TRACKER_DEBUG then
        print(string.format("[BuffTracker] Icon %s (%s): hasData=%s, animation=%s",
            tostring(barIndex), trackedBuff.name or "?", tostring(hasData), iconAnimation))
    end
end
...
function ResourceBars:UpdateSingleTrackedBuffSound(barIndex, trackedBuff, globalCfg)
    ...
    if BUFF_TRACKER_DEBUG then
        print(string.format("[BuffTracker] Sound %s (%s): hasData=%s, trigger=%s",
            tostring(barIndex), trackedBuff.name or "?", tostring(hasData), soundTrigger))
    end
end
...
function ResourceBars:UpdateSingleTrackedBuffText(barIndex, trackedBuff, globalCfg)
    ...
    -- ============================================================
    local showDurationText = settings.showDurationText or false
    if showDurationText and textFrame.DurationText then
        local dtFont = settings.durationTextFont
        local dtFontPath = dtFont and LSM:Fetch("font", dtFont) or STANDARD_TEXT_FONT
        local dtSize = settings.durationTextSize or 10
        local dtAlign = settings.durationTextAlign or "LEFT"
        local dtX = settings.durationTextX or 4
        local dtY = settings.durationTextY or 0
        local dtColor = settings.durationTextColor or { 1, 1, 1, 1 }
        local dtDecimals = settings.durationDecimals or 1
        local dtWarning = settings.durationWarningEnabled or false
        ...
        textFrame.DurationText:ClearAllPoints()
        textFrame.DurationText:SetPoint(dtAlign, textFrame.Text, "RIGHT", DDingUI:Scale(dtX), DDingUI:Scale(dtY))
        textFrame.DurationText:SetJustifyH(dtAlign)
            ...
            if not textFrame._hasDurationUpdate then
                RegisterDurationUpdate(textFrame, function(self, elapsed)
                    ...
                    if data.isManualMode then
                        ...
                        if expiresAt then
                            local remaining = math_max(0, expiresAt - GetTime())
                            textFrame.DurationText:SetText(FormatDuration(remaining, data.durationDecimals))
                            if data.warningEnabled then
                                if remaining <= data.warningThreshold then
                                    local wc = data.warningColor
                                    textFrame.DurationText:SetTextColor(wc[1] or 1, wc[2] or 0.2, wc[3] or 0.2, wc[4] or 1)
                                else
                                    local nc = data.normalColor
                                    textFrame.DurationText:SetTextColor(nc[1] or 1, nc[2] or 1, nc[3] or 1, nc[4] or 1)
                                end
                            end
                        end
                        return
        ...
        elseif isManualMode and (manualStackCount or 0) > 0 then
            ...
            textFrame.DurationText:Show()
        elseif isInPreviewMode or isInMoverMode then
            textFrame.DurationText:SetText(FormatDuration(18.5, dtDecimals))
            textFrame.DurationText:Show()
        else
    ...
    if BUFF_TRACKER_DEBUG then
        print(string.format("[BuffTracker] Text %s (%s): hasData=%s, mode=%s",
            tostring(barIndex), trackedBuff.name or "?", tostring(hasData), textDisplayMode))
    end
end
...
SlashCmdList["DDINGBUFF"] = function(msg)
    ...
    if cdID then
        ...
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: Testing cdID: " .. cdID)
        ...
        BUFF_TRACKER_DEBUG = not BUFF_TRACKER_DEBUG
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: Debug: " .. (BUFF_TRACKER_DEBUG and "ON" or "OFF"))
    end
end
...
SlashCmdList["BTSCAN"] = function(msg)
    print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: Scanning available buffs...")
    ...
    for i, buff in ipairs(buffs) do
        ...
        print(string.format("    %s. |cffffffff%s|r (cdID:%s, spellID:%s)%s [%s]",
            tostring(i), buff.spellName or "Unknown", tostring(buff.cooldownID), tostring(buff.spellID or 0), chargeStr, buff.source))
    end
...
SlashCmdList["BTRESET"] = function(msg)
    if not DDingUI.db then
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cffff0000 BuffTracker config not found!")
        return
    end

    -- db.global.trackedBuffsPerSpec 사용 (계정 공유)
    local globalStore = DDingUI.db.global and DDingUI.db.global.trackedBuffsPerSpec
    local rootCfg = DDingUI.db.profile and DDingUI.db.profile.buffTrackerBar

    if msg == "all" then
        -- Reset all tracked buffs (global + legacy)
        if globalStore then
            DDingUI.db.global.trackedBuffsPerSpec = {}
        end
        if rootCfg then
        ...
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cff00ff00 All tracked buffs cleared!")
    elseif msg == "spec" or msg == "" then
        -- Reset current spec's tracked buffs only
        local specID = GetCurrentSpecID()
        if specID and globalStore then
            globalStore[specID] = {}
            print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cff00ff00 Current spec (ID: " .. specID .. ") tracked buffs cleared!")
        else
            print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cffff0000 Could not determine current spec!")
        end
    elseif msg == "legacy" then
        ...
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cff00ff00 Cleaned " .. cleaned .. " legacy spec entries! (Now using SpecProfiles system)")
    elseif msg == "help" then
        print("|cffffffffDDing|r|cffffa300UI|r |cffe6731fCDM|r: |cffffcc00BuffTracker Reset|r")
