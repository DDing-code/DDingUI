...
if not DDingUI then
	error("DDingUI not found! PixelPerfect.lua must load after Main.lua")
end
...
function DDingUI:RefreshGlobalFX()
...
function DDingUI:IsEyefinity(width, height)
	if DDingUI.db and DDingUI.db.profile and DDingUI.db.profile.general and DDingUI.db.profile.general.eyefinity and width >= 3840 then
...
function DDingUI:IsUltrawide(width, height)
	if DDingUI.db and DDingUI.db.profile and DDingUI.db.profile.general and DDingUI.db.profile.general.ultrawide and width >= 2560 then
...
function DDingUI:UIMult()
	DDingUI.mult = DDingUI.perfect
end

function DDingUI:PixelBestSize()
	return max(0.4, min(1.15, DDingUI.perfect))
end

function DDingUI:PixelScaleChanged(event)
	if event == 'UI_SCALE_CHANGED' then
		DDingUI.physicalWidth, DDingUI.physicalHeight = GetPhysicalScreenSize()
		DDingUI.resolution = format('%dx%d', DDingUI.physicalWidth, DDingUI.physicalHeight)
		DDingUI.perfect = 768 / DDingUI.physicalHeight
	end

	DDingUI:UIMult()
	
	if DDingUI.ActionBars and DDingUI.ActionBars.RefreshAll then
		DDingUI.ActionBars:RefreshAll()
	end
end

function DDingUI:Scale(x)
	local m = DDingUI.mult
	if m == 1 or x == 0 then
		return x
	else
...
function DDingUI:ScaleBorder(borderSize)
