...
local function GetSnappedMetricsPx(size, spacing)
    ...
    local gapPx = CDM_C.GetCooldownIconGapPixels and CDM_C.GetCooldownIconGapPixels(spacing, UIParent) or math.max(0, ToPixelCount(spacing or 0))
...
local function ShouldDampenReanchorRetry(cdm)
    if not cdm then return false end
    return cdm.loadingScreenActive or not cdm.loginFinished
end
...
local function QueueReanchorRetry(cdm, vName, delay)
    ...
    local dampen = ShouldDampenReanchorRetry(cdm)
    local maxRetries = dampen and 4 or 6
    if state.count >= maxRetries then
        return
    end
    ...
    local retryDelay = delay or 0.1
    if dampen and retryDelay < 0.05 then
        retryDelay = 0.05
    end

    C_Timer.After(retryDelay, function()
        state.pending = false
        cdm:QueueViewer(vName, not ShouldDampenReanchorRetry(cdm))
    end)
end
