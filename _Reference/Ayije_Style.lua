...
local function GetUIParentPixelSize()
...
local function SnapToPixel(value)
    local pixel = GetUIParentPixelSize()
    if not pixel then return value end
    return CDM_C.PixelPerfect(value / pixel) * pixel
end

local function GetUIPixelSize()
    return GetUIParentPixelSize() or 1
end
...
local function GetPixelForRegion(region)
    if PixelUtil and PixelUtil.ConvertPixelsToUIForRegion and region then
        local ok, px = pcall(PixelUtil.ConvertPixelsToUIForRegion, 1, region)
        if ok and type(px) == "number" and px > 0 then
            return px
        end
    end
    return GetUIPixelSize()
end

local function IsPixelIconBorderMode()
    return CDM_C.IsPixelIconBorderMode and CDM_C.IsPixelIconBorderMode() or false
end
...
local function ApplyPixelIconBorder(frameData, frame, color)
    ...
    local px = GetPixelForRegion(overlay)
    ...
    if needsVisualUpdate then
        ...
        if isCooldown then
            if frame.ChargeCount then
                frame.ChargeCount:SetFrameLevel(frame:GetFrameLevel() + 4)
            end
        ...
        if isBuff then
            if frame.Applications then
                frame.Applications:SetFrameLevel(frame:GetFrameLevel() + 4)
            end
...
function CDM:ApplyBarStyle(frame, vName, iconPositionOverride, frameWidthOverride, frameHeightOverride)
    ...
    local barHeight = styleCache.buffBarHeight
    local iconGap = styleCache.buffBarIconGap
    ...
    if iconFrame then
        if iconPosition == "HIDDEN" then
            ...
            if borderActive then
                ...
                if BORDER and BORDER.CreateBorder and borderForce then
    ...
    if bar then
        ...
        elseif iconPosition == "RIGHT" then
            bar:SetPoint("LEFT", frame, "LEFT", 0, 0)
            bar:SetPoint("RIGHT", iconFrame or frame, iconFrame and "LEFT" or "RIGHT", iconFrame and -iconGap or 0, 0)
        else
            bar:SetPoint("LEFT", iconFrame or frame, iconFrame and "RIGHT" or "LEFT", iconFrame and iconGap or 0, 0)
            bar:SetPoint("RIGHT", frame, "RIGHT", 0, 0)
        end
        ...
        if wantsNameText or wantsDurationText then
            ...
            if nameText then
                ...
                if showName then
                    ...
                    nameText:SetPoint("LEFT", bar, "LEFT", nameOffsetX, nameOffsetY)
                    nameText:SetPoint("RIGHT", bar, "RIGHT", -30, nameOffsetY)
                else
                    nameText:Hide()
                    nameText:SetAlpha(0)
                end
            ...
            if durationText then
                ...
                if showDuration then
                    ...
                    durationText:SetPoint("RIGHT", bar, "RIGHT", durationOffsetX, durationOffsetY)
                else
                    durationText:Hide()
        ...
        if appText then
            local canShowApplications = showApplications and iconPosition ~= "HIDDEN"
            if canShowApplications then
                if not frameData.iconTextContainer then
                    frameData.iconTextContainer = CreateFrame("Frame", nil, iconFrame)
                    frameData.iconTextContainer:SetAllPoints(iconFrame)
                    frameData.iconTextContainer:SetFrameLevel(iconFrame:GetFrameLevel() + 4)
                end

                frameData.iconTextContainer:Show()
                InstallBuffBarVisibilityShowHook(frameData, "cdmAppHooked", appText, "buffBarShowApplications")
                appText:SetParent(frameData.iconTextContainer)
                appText:SetAlpha(1)
                appText:Show()
                ...
                appText:SetPoint("CENTER", iconFrame, appPosition, appOffsetX, appOffsetY)
            else
                if frameData.iconTextContainer then frameData.iconTextContainer:Hide() end
                appText:Hide()
                appText:SetAlpha(0)
            end
        end
        ...
        if borderActive then
            if not frameData.barBorderFrame then
                frameData.barBorderFrame = CreateFrame("Frame", nil, bar, "BackdropTemplate")
                frameData.barBorderFrame:SetAllPoints()
                frameData.barBorderFrame:SetFrameLevel(bar:GetFrameLevel() + 2)
            end
