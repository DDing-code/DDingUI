...
-- Buff icons and buff bars share Blizzard aura data, but keep separate placement/styling pipelines.
-- This policy only aligns queue/reanchor timing (coalesced item hooks + post-layout stabilization).
local BUFF_LIKE_VIEWER_HOOK_POLICY = {
    [VIEWERS.BUFF] = {
        queueOnRefreshData = true,
        queueOnPostLayout = true,
        refreshDataImmediate = true,
        postLayoutImmediate = true,
        itemHookImmediate = false,
        poolReleaseImmediate = true, -- preserve icon collapse responsiveness
    },
    [VIEWERS.BUFF_BAR] = {
        queueOnRefreshData = true,
        queueOnPostLayout = true,
        ...
        postLayoutImmediate = true,
        itemHookImmediate = false,
        poolReleaseImmediate = true,
    },
}

local function GetBuffLikeViewerHookPolicy(name)
    return BUFF_LIKE_VIEWER_HOOK_POLICY[name]
end

local function QueueBuffLikeViewerFromItemHooks(name)
    local policy = GetBuffLikeViewerHookPolicy(name)
    local immediate = policy and policy.itemHookImmediate or false

    CDM:QueueViewer(name, immediate)
end
...
local function HookViewerLayout(viewer, name)
    ...
    local buffLikePolicy = GetBuffLikeViewerHookPolicy(name)
    local queueOnRefreshData = buffLikePolicy and buffLikePolicy.queueOnRefreshData or false
    local queueOnPostLayout = (not queueOnRefreshData) or (buffLikePolicy and buffLikePolicy.queueOnPostLayout)
    local queueOnRefreshLayout = (name == VIEWERS.ESSENTIAL) or (name == VIEWERS.UTILITY)
    local function QueuePostLayoutViewer()
        if queueOnPostLayout then
            -- Buff-like viewers can queue from per-item hooks before Blizzard layout has settled.
            CDM:QueueViewer(name, (buffLikePolicy and buffLikePolicy.postLayoutImmediate) ~= false)
        end
    end

    if viewer.RefreshData and queueOnRefreshData then
        hooksecurefunc(viewer, "RefreshData", function()
            CDM:QueueViewer(name, buffLikePolicy.refreshDataImmediate ~= false)
        end)
    end
...
function CDM:QueueViewer(name, immediate, version)
    ...
    if name == VIEWERS.BUFF then
        if self.MarkBuffCenteringDirty then
            self:MarkBuffCenteringDirty()
        end
        if self.MarkSecTertCenteringDirty then
            self:MarkSecTertCenteringDirty()
        end
...
function CDM:SetupViewer(vName)
    ...
    if v.itemFramePool then
        ...
        if (vName == VIEWERS.BUFF or vName == VIEWERS.BUFF_BAR or vName == VIEWERS.UTILITY) and not CDM_ViewerPoolReleaseHooked[v] then
            ...
            hooksecurefunc(v.itemFramePool, "Release", function()
                local buffLikePolicy = GetBuffLikeViewerHookPolicy(vName)
                local immediate = buffLikePolicy and buffLikePolicy.poolReleaseImmediate or false
    ...
    if not CDM_ViewerOnShowHooked[v] then
        ...
        -- [FIX] Blizzard가 뷰어를 Hide()할 때 (전문화 변경/레벨업 등)
        -- 편집모드 앵커 목록에서 사라지지 않도록 지연 재표시
        -- C_Timer.After(0)을 사용해 블리자드 ManagedFrame 재배치를 우회
        v:HookScript("OnHide", function()
            if InCombatLockdown() then return end
            -- 로딩 화면 중에는 재표시하지 않음
            if CDM.loadingScreenActive then return end
            C_Timer.After(0, function()
                if InCombatLockdown() then return end
