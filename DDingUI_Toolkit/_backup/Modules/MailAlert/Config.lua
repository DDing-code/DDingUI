--[[
    DDingToolKit - MailAlert Config
    MailAlert Settings Panel
]]
...
local function GetSoundOptions()
    return ns:GetSoundOptions(true, L["MAILALERT_DEFAULT_SOUND"] or "Whisper (Default)", "")
end
...
function MailAlert:CreateConfigPanel(parent)
    ...
    -- Sound alert
    local soundCheckbox = UI:CreateCheckbox(panel, L["MAILALERT_SOUND_ENABLED"], function(checked)
    ...
    -- Screen alert
    local screenCheckbox = UI:CreateCheckbox(panel, L["MAILALERT_SCREEN_ALERT"], function(checked)
    ...
    -- Chat alert
    local chatCheckbox = UI:CreateCheckbox(panel, L["MAILALERT_CHAT_ALERT"], function(checked)
    ...
    -- ===== Condition Settings =====
    local conditionHeader = UI:CreateSectionHeader(panel, L["MAILALERT_CONDITION_SETTINGS"])
    conditionHeader:SetPoint("TOPLEFT", leftCol, yOffset)
    yOffset = yOffset - 30

    -- Hide in combat
    local combatCheckbox = UI:CreateCheckbox(panel, L["MAILALERT_HIDE_IN_COMBAT_DESC"], function(checked)
        self.db.hideInCombat = checked
    end)
    combatCheckbox:SetPoint("TOPLEFT", leftCol + 5, yOffset)
    combatCheckbox:SetChecked(self.db.hideInCombat)
    yOffset = yOffset - 30

    -- Hide in instance
    local instanceCheckbox = UI:CreateCheckbox(panel, L["MAILALERT_HIDE_IN_INSTANCE_DESC"], function(checked)
        self.db.hideInInstance = checked
    end)
    instanceCheckbox:SetPoint("TOPLEFT", leftCol + 5, yOffset)
    instanceCheckbox:SetChecked(self.db.hideInInstance)
    yOffset = yOffset - 40

    -- Cooldown
    local cooldownSlider = UI:CreateSlider(panel, L["ALERT_COOLDOWN"], 10, 300, 10, function(value)
        self.db.cooldown = value
    end)
    cooldownSlider:SetPoint("TOPLEFT", leftCol + 5, yOffset)
    ...
    channelLabel:SetPoint("TOPLEFT", rightCol + 5, rightYOffset)
    ...
    channelDropdown:SetPoint("TOPLEFT", rightCol + 5, rightYOffset)
