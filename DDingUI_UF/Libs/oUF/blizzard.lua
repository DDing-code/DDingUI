...
-- 재귀 방지: hooksecurefunc 체인에서 ElvUI 등 다른 애드온과 SetParent 무한 재귀 방지
-- C_Timer.After(0)으로 현재 프레임 틱 동안 플래그 유지, 다음 틱에 해제
local resettingFrames = {}
local function resetParent(self, parent)
	if(parent == hiddenParent) then return end
	if(resettingFrames[self]) then return end

	resettingFrames[self] = true
	if(InCombatLockdown() and self:IsProtected()) then
		looseFrames[self] = true
	else
		self:SetParent(hiddenParent)
	end
	C_Timer.After(0, function() resettingFrames[self] = nil end)
end
...
function oUF:DisableBlizzard(unit)
	...
	elseif(unit:match('arena%d?$')) then
		if(not isArenaHooked) then
			...
			-- doNotReparent = true: CompactArenaFrame은 EditMode 관리 컨테이너이므로
			-- SetParent 훅이 ElvUI 등 다른 애드온의 SetParent 훅과 무한 재귀를 일으킴
			-- Hide() + UnregisterAllEvents()로 충분히 비활성화됨
			handleFrame(CompactArenaFrame, true)
