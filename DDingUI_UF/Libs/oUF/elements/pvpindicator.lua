...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	-- [12.0.1] UnitIsPVP/UnitIsPVPFreeForAll can return secret boolean
	local isPVPFFA = not (issecretvalue and issecretvalue(UnitIsPVPFreeForAll(unit))) and UnitIsPVPFreeForAll(unit)
	local isPVP = not (issecretvalue and issecretvalue(UnitIsPVP(unit))) and UnitIsPVP(unit)
	if(isPVPFFA) then
		status = 'FFA'
