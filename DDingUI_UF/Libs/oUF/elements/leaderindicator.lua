...
local function Update(self, event)
	...
	* self - the LeaderIndicator element
	...
	if(IsInInstance()) then
		isLeader = SafeBoolTest(UnitIsGroupLeader(unit)) -- [12.0.1]
	else
		isLeader = SafeBoolTest(UnitLeadsAnyGroup(unit)) -- [12.0.1]
	end
	...
	if(isLeader) then
		...
		elseif(element._iconSetTexture) then
			-- [REFACTOR] 커스텀 아이콘 세트 텍스처 사용
			element:SetTexture(element._iconSetTexture)
