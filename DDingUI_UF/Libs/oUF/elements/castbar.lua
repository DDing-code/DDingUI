...
local function CastStart(self, event, unit)
	if(not unit) then return end -- [FIX] nil unit 방어 (PlayerCastingBarFrame taint)
...
local function CastUpdate(self, event, unit, _, _, castID)
	if(not unit) then return end -- [FIX] nil unit 방어
	local element = self.Castbar
	if(not (element.ShouldShow or ShouldShow) (element, unit)) then
		return
	end
...
local function Enable(self, unit)
	...
	if(element and unit and not unit:match('%wtarget$')) then
		...
		if(self.unit == 'player' and not (self.hasChildren or self.isChild or self.isNamePlate)) then
			...
			-- [FIX] EditMode가 PLAYER_ENTERING_WORLD 재등록 시 nil unit 크래시 방지
			if(not PlayerCastingBarFrame._dduiGuarded) then
				PlayerCastingBarFrame._origOnEvent = PlayerCastingBarFrame:GetScript('OnEvent')
				if(PlayerCastingBarFrame._origOnEvent) then
					local orig = PlayerCastingBarFrame._origOnEvent
					PlayerCastingBarFrame:SetScript('OnEvent', function(self, event, ...)
						if(not self.unit and not (self.GetUnit and self:GetUnit())) then return end
						return orig(self, event, ...)
					end)
				end
				-- [FIX] EditMode/Blizzard 코드가 이벤트 재등록하는 것 차단
				hooksecurefunc(PlayerCastingBarFrame, 'RegisterEvent', function(self, event)
					if(self._dduiEventsBlocked) then
						self:UnregisterEvent(event)
					end
				end)
				hooksecurefunc(PlayerCastingBarFrame, 'RegisterUnitEvent', function(self, event, ...)
					if(self._dduiEventsBlocked) then
						self:UnregisterEvent(event)
					end
				end)
				PlayerCastingBarFrame._dduiGuarded = true
			end
			PlayerCastingBarFrame._dduiEventsBlocked = true
...
local function Disable(self)
	...
	if(element) then
		...
		if(self.unit == 'player' and not (self.hasChildren or self.isChild or self.isNamePlate)) then
			-- [FIX] 가드 OnEvent는 유지 (unit nil 방어 → 영구)
			-- 원본 복원 시 Enable/Disable 사이클에서 가드가 빠져 PLAYER_ENTERING_WORLD + nil unit 크래시
			-- 가드는 self.unit nil일 때만 차단, 정상 시 투명 통과
			PlayerCastingBarFrame._dduiEventsBlocked = nil
