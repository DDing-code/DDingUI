...
-- [12.0.1] secret boolean safe check
...
local function UpdateColor(self, event, unit)
	...
	local color
	if(element.colorDisconnected and not SafeBoolTest(UnitIsConnected(unit))) then
		color = self.colors.disconnected
	elseif(element.colorTapping and not UnitPlayerControlled(unit) and UnitIsTapDenied(unit)) then
		color = self.colors.tapped
	elseif(element.colorThreat and not UnitPlayerControlled(unit)) then
		-- [12.0.1] UnitThreatSituation secret number 방어
		local threat = UnitThreatSituation('player', unit)
		if threat and not (issecretvalue and issecretvalue(threat)) then
	...
	elseif(element.colorReaction) then
		-- [12.0.1] UnitReaction secret number 방어
...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	if(SafeBoolTest(UnitIsConnected(unit))) then -- [12.0.1]
