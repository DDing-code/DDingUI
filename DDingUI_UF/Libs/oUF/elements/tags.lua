...
local _ENV = {
	...
	-- [12.0.1] secret boolean safe test for tag functions
	SafeBool = function(val)
...
local tagStrings = {
	...
	['dead'] = [[function(u)
		if(SafeBool(UnitIsDead(u))) then
			return 'Dead'
		elseif(SafeBool(UnitIsGhost(u))) then
			return 'Ghost'
		end
	end]],
	...
	['leader'] = [[function(u)
		if(SafeBool(UnitIsGroupLeader(u))) then
			return 'L'
		end
	end]],
	...
	['offline'] = [[function(u)
		if(SafeBool(UnitIsConnected(u)) == false) then
			return 'Offline'
		end
	end]],
	...
	['status'] = [[function(u)
		...
		elseif(SafeBool(UnitIsGhost(u))) then
			return 'Ghost'
		elseif(SafeBool(UnitIsConnected(u)) == false) then
			return 'Offline'
		else
	...
	if(not func) then
				...
				if(tag) then
					if(prefixEnd ~= 1 or suffixStart - suffixEnd ~= 1) then
						...
						tagFunc = function(unit, realUnit)
							...
							if(str and (oUF:IsSecretValue(str) or str ~= '')) then
		...
		func = function(self)
			...
			for i, f in next, funcs do
				local ok, result = pcall(f, unit, realUnit)
				buffer[i] = (ok and result) or ''
			end
