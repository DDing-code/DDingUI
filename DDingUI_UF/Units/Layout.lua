...
local _, ns = ...
local oUF = ns.oUF
-- [STANDALONE] oUF 없을 때 TextFormats.Methods를 폴백으로 사용
if not oUF then
	oUF = { Tags = { Methods = (ns.TextFormats and ns.TextFormats.Methods) or {}, Events = {} } }
end
...
local function MigrateTagString(tagString)
	...
	local methods = (oUF and oUF.Tags and oUF.Tags.Methods) or (ns.TextFormats and ns.TextFormats.Methods) or {}
...
local function IsValidCustomTag(tagString)
	...
	for tag in migrated:gmatch("%[([^%]]+)%]") do
		local allMethods = (oUF and oUF.Tags and oUF.Tags.Methods) or (ns.TextFormats and ns.TextFormats.Methods) or {}
		if not allMethods[tag] then
			return false
		end
	end
	return true
...
local function CreateHealthPrediction(self, settings) -- [FIX] 미적용 옵션 연결: settings 파라미터 추가
	...
	-- overShieldBar: 풀피/초과 보호막 오버레이 (DandersFrames ATTACHED_OVERFLOW 패턴)
	-- 보호막이 부족 체력을 초과할 때 오른쪽에서 왼쪽으로 ReverseFill 표시
	local overShieldBar = CreateFrame("StatusBar", nil, health)
	overShieldBar:SetStatusBarTexture(sbTex)
	overShieldBar:GetStatusBarTexture():SetVertexColor(sbColor[1], sbColor[2], sbColor[3], (sbColor[4] or 0.4) * 0.85)
	overShieldBar:SetAllPoints(health)
	overShieldBar:SetReverseFill(true)
	overShieldBar:SetFrameLevel(health:GetFrameLevel() + 2)
	overShieldBar:Hide()
	...
	local hdData = {
		...
		overShieldBar = overShieldBar,  -- [FIX] 초과 보호막 오버레이 (ATTACHED_OVERFLOW)
	...
	-- [STANDALONE] oUF Enable과 동일: Calculator 초기화
	-- oUF가 없으면 여기서 직접 Calculator를 생성하여 hdData.values에 저장
	if not ns.oUF or not ns.oUF.objects then
		if CreateUnitHealPredictionCalculator then
			hdData.values = CreateUnitHealPredictionCalculator()
			if hdData.damageAbsorbClampMode and hdData.values.SetDamageAbsorbClampMode then
				hdData.values:SetDamageAbsorbClampMode(hdData.damageAbsorbClampMode)
			end
			if hdData.incomingHealOverflow and hdData.values.SetIncomingHealOverflowPercent then
				hdData.values:SetIncomingHealOverflowPercent(hdData.incomingHealOverflow)
			end
		end
		hdData.__owner = self
	end
...
function Layout:StyleUnit(self, unit)
	...
	-- Health Prediction (보호막 + 힐예측 + 힐흡수)
	-- [FIX] 모든 유닛에 대해 생성 (기존: player/party/raid/focus만 → target 등 누락 버그)
	CreateHealthPrediction(self, settings)
