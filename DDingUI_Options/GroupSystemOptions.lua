...
local function IsDefaultTrackedEntry(entry)
    -- The assigned preview mirrors frames that GroupSystem can actually render.
...
local function CollectCDMRowsForGroup(groupName, allEntries, skipSpellNames, trackedOnly)
    ...
    local function BuildRuntimeEntry(runtimeEntry, fallbackOrder)
        ...
        local entry = {
            cooldownID = cooldownID,
            spellID = candidates[1] or frameSpellID,
...
local function BuildAssignedSpellsArgs(groupName)
    ...
    -- The assigned preview follows the same classified runtime list rendered
    -- by GroupSystem. Inactive catalog entries stay in the add picker.
...
function DDingUI:ShowGroupIconAddPopup(owner, groupName, settings, unassignedRows, onDone, addMode)
    ...
    if not popup then
        ...
        popup.edit = CreateFrame("EditBox", nil, popup, "BackdropTemplate")
        popup.edit:SetBackdrop({ bgFile = FLAT, edgeFile = FLAT, edgeSize = 1 })
        popup.edit:SetBackdropColor(0.035, 0.045, 0.055, 0.98)
        popup.edit:SetBackdropBorderColor(0.18, 0.24, 0.26, 1)
        popup.edit:SetAutoFocus(false)
        popup.edit:SetHeight(24)
        popup.edit:SetFont(gf, 11, "")
        popup.edit:SetTextColor(0.92, 0.94, 0.95, 1)
        popup.edit:SetTextInsets(6, 6, 0, 0)
        popup.edit:SetScript("OnEditFocusGained", function(self)
            self:SetBackdropBorderColor(accentR, accentG, accentB, 0.9)
        end)
        popup.edit:SetScript("OnEditFocusLost", function(self)
            self:SetBackdropBorderColor(0.18, 0.24, 0.26, 1)
        end)
        popup.edit.placeholder = popup.edit:CreateFontString(nil, "OVERLAY")
        popup.edit.placeholder:SetPoint("LEFT", popup.edit, "LEFT", 6, 0)
