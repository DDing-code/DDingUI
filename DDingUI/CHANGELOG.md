...
- Made the DDingUI TOC version the single source of truth for runtime and GUI version labels, preventing stale hardcoded versions from remaining in the settings window.
...
- 런타임과 설정창의 버전 표시는 DDingUI TOC 버전만 따르도록 통일하여, 하드코딩된 이전 버전이 설정창에 남는 문제를 방지했습니다.
