...
DDingUI.AuraOverride = DDingUI.AuraOverride or {}
local AuraOverride = DDingUI.AuraOverride

-- Reference shared weak tables (avoids tainting Blizzard frames)
local IconViewers = DDingUI.IconViewers
local iconData = IconViewers and IconViewers._iconData or setmetatable({}, { __mode = "k" })
local texData = IconViewers and IconViewers._texData or setmetatable({}, { __mode = "k" })
local function GetViewerState(viewer)
    if IconViewers and IconViewers._GetViewerState then
        return IconViewers._GetViewerState(viewer)
    end
    local state = IconViewers and IconViewers._viewerState and IconViewers._viewerState[viewer]
    if not state then state = {}; if IconViewers and IconViewers._viewerState then IconViewers._viewerState[viewer] = state end end
    return state
end
...
local function GetViewerSettings(viewerName)
    ...
    local settings = DDingUI.db.profile.viewers[viewerName]
...
local function ApplyDesaturationForAuraActive(iconFrame, desaturate)
    ...
    -- Set the force value flag in weak table - hooks will enforce this
    local id = GetIconData(iconFrame)
    if desaturate then
        id.forceDesatValue = 1
    else
        id.forceDesatValue = nil
    end
...
local function HookCooldownFrame(iconFrame, viewerName)
    ...
    local id = GetIconData(iconFrame)
    if id.auraOverrideHooked then return end

    id.auraOverrideHooked = true
    id.viewerName = viewerName
    ...
    if iconTexture then
        local td = texData[iconTexture]
        if not (td and td.desatHooked) then
            if not td then td = {}; texData[iconTexture] = td end
            td.desatHooked = true
            td.parentFrame = iconFrame
            ...
            if iconTexture.SetDesaturated then
                hooksecurefunc(iconTexture, "SetDesaturated", function(self, desaturated)
                    local tdd = texData[self]
                    if not tdd then return end
                    local pf = tdd.parentFrame
                    if not pf then return end
                    local pid = iconData[pf]
                    if pid and pid.bypassDesatHook then return end

                    -- If we have a forced desaturation value (for ignoreAuraOverride), enforce it
                    local forceValue = pid and pid.forceDesatValue
                    if forceValue ~= nil and self.SetDesaturation then
                        if not pid then pid = GetIconData(pf) end
                        pid.bypassDesatHook = true
                        self:SetDesaturation(forceValue)
                        pid.bypassDesatHook = false
                    end
                end)
            end
    ...
    hooksecurefunc(cooldown, "SetCooldown", function(self, startTime, duration)
        ...
        local pid = iconData[parentFrame]
        if not pid or not pid.viewerName then return end
        if pid.bypassCooldownHook then return end

        local vName = pid.viewerName
        local ignoreAuraOverride = GetViewerSettings(vName)
                ...
                if spellID and icon.Cooldown then
                    ...
                    if isChargeSpell and C_Spell.GetSpellChargeDuration then
                        ...
                        if ok and cooldownInfo and cooldownInfo.duration and cooldownInfo.startTime then
                            ...
                            -- Set force desaturation value - hooks will enforce it
                            GetIconData(icon).forceDesatValue = 1
...
local _viewerNames = {"EssentialCooldownViewer", "UtilityCooldownViewer"}
local _refreshPending = false

local function _DoRefreshViewers()
    _refreshPending = false
    for _, viewerName in ipairs(_viewerNames) do
...
eventFrame:SetScript("OnEvent", function(self, event, unit)
    if event == "UNIT_AURA" or event == "SPELL_UPDATE_COOLDOWN" then
        if not _refreshPending then
            ...
            C_Timer.After(0.01, _DoRefreshViewers)
        end
    end
end)
...
function AuraOverride:Initialize()
    ...
    for _, viewerName in ipairs(viewers) do
        ...
        if viewer then
            ...
            if not GetViewerState(viewer).auraOverrideHooked then
                GetViewerState(viewer).auraOverrideHooked = true
    ...
    if DDingUI.IconViewers and DDingUI.IconViewers.SkinIcon then
        ...
        function DDingUI.IconViewers:SkinIcon(icon, settings)
            ...
            for name, viewerSettings in pairs(DDingUI.db.profile.viewers) do
