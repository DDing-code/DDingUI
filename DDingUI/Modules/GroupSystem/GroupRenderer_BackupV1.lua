                ...
                icon:HookScript("OnHide", function(self)
                    ...
                    if not p._ddLayoutPending then
                        ...
                        C_Timer.After(0.03, function()
                            ...
                            local vSettings2 = GetViewerSettings(vn2)
                            if vSettings2 then
                                GroupRenderer:LayoutGroup(p, vSettings2, vn2)
                            end
...
function GroupRenderer:LayoutGroup(frame, viewerSettings, viewerName)
    ...
    if InCombatLockdown() then
        -- 전투 중: 크기를 기록만 하고 전투 종료 후 적용
        frame._pendingW = finalW
        frame._pendingH = finalH
        -- FrameController._postCombatQueue에 지연 적용 등록
        local fc = GetFC()
        if fc and fc._postCombatQueue then
            local frameName = frame:GetName() or tostring(frame)
            fc._postCombatQueue["layout_" .. frameName] = function()
                if frame and frame.SetSize then
                    frame:SetSize(frame._pendingW or finalW, frame._pendingH or finalH)
                    frame._pendingW = nil
                    frame._pendingH = nil
                end
            end
...
function GroupRenderer:UpdateDynamicGroup(groupName, groupSettings, frame)
    ...
    -- Show/Hide 결정 (컨테이너는 Taint로 전투 중 Protected)
    if not InCombatLockdown() then
        if idx > 0 then
            frame:Show()
        else
