...
local function FindAuraGlobally(sID)
    if not sID or sID <= 0 then return nil, nil end
    local aura = C_UnitAuras.GetPlayerAuraBySpellID(sID)
    if aura then return aura, "player" end

    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("target", i, "PLAYER|HARMFUL")
        if not b then break end
        if b.spellId == sID then return b, "target" end
    end
    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("target", i, "PLAYER|HELPFUL")
        if not b then break end
        if b.spellId == sID then return b, "target" end
    end
    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("player", i, "HARMFUL")
        if not b then break end
        if b.spellId == sID then return b, "player" end
    end
    return nil, nil
end
...
local function FindAuraGloballyByName(name)
    ...
    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("player", i, "HELPFUL")
        if not b then break end
        if b.name == name then return b, "player" end
    end
    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("player", i, "HARMFUL")
        if not b then break end
    ...
    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("target", i, "PLAYER|HARMFUL")
        if not b then break end
        if b.name == name then return b, "target" end
    end
    for i = 1, 255 do
        local b = C_UnitAuras.GetAuraDataByIndex("target", i, "PLAYER|HELPFUL")
        if not b then break end
...
local function ResolveTrackedStacks(cooldownID, frame, isManualMode, manualStackCount, spellID, spellName)
    ...
    if BUFF_TRACKER_DEBUG and (cooldownID > 0 or (resolvedSpellID and resolvedSpellID > 0)) then
        pcall(function()
            print(string.format("|cffff8800[BT]|r cdID=%s res=%s stacks=%s auraID=%s unit=%s",
                tostring(cooldownID), tostring(resolvedSpellID),
                tostring(trackedStacks), tostring(auraInstanceID), tostring(unit)))
        end)
    end
