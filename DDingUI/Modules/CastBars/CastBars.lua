...
-- Create namespace
DDingUI.CastBars = DDingUI.CastBars or {}
local CastBars = DDingUI.CastBars

-- Cache frequently used globals for performance (especially in OnUpdate hot path)
...
local function CreateBorder(frame)
	...
	local borderSize = (DDingUI.ScaleBorder and DDingUI:ScaleBorder(1)) or math.floor((DDingUI:Scale(1) or 1) + 0.5)
...
local function FormatCastTime(current, total, format)
    format = format or "current/total"
    local remaining = total - current

    if format == "(current)/(total)" then
        return string_format("(%.1f)/(%.1f)", current, total)
    elseif format == "current:total" then
        return string_format("%.1f:%.1f", current, total)
    elseif format == "remaining" then
        return string_format("%.1f", remaining)
    elseif format == "remaining/total" then
        return string_format("%.1f/%.1f", remaining, total)
    else -- "current/total" (default)
        return string_format("%.1f/%.1f", current, total)
    end
end

CastBars.FormatCastTime = FormatCastTime
...
local function CastBar_OnUpdate(frame, elapsed)
    ...
    if frame.isEmpowered and frame.numStages and frame.numStages > 0 then
        ...
        elseif #stagePercentages > 0 then
            ...
            for i = 1, math_min(#stagePercentages - 2, frame.numStages) do  -- Exclude blank period and hold-at-max
                ...
                if i == math_min(#stagePercentages - 2, frame.numStages) then
                    currentStage = i
                    stageStartPercent = cumulative
                    stageEndPercent = 1.0
                end
            ...
            if progressPercent < stage1StartPercent then
                ...
                currentStage = math_min(math_max(1, math_ceil(adjustedProgress / stageSize)), frame.numStages)
        ...
        if frame.empoweredSegments and frame.empoweredSegmentFills then
            ...
            if stage1StartPercent > 0 then
                ...
                if blankSegmentBg and blankSegmentFill then
                    ...
                    if progressPercent < stage1StartPercent then
                        ...
                        local fillWidth = math_max(0, blankSegmentWidth * blankProgress)
            ...
            if showStageColors then
                ...
                local holdAtMaxStartPercent = math_min(math_max(lastStageEndPercent, stage1StartPercent), 0.95)
                    ...
                    if segmentFill then
                        ...
                        if i == frame.numStages then
                            ...
                            if progressPercent >= segmentStart and progressPercent < holdAtMaxStartPercent then
                                ...
                                if segmentDuration > 0 then
                                    local stageProgress = math_min(1.0, math_max(0, (progressPercent - segmentStart) / segmentDuration))
                                    local fillWidth = math_max(0, segmentWidth * stageProgress)
                        ...
                        elseif i == currentStage and currentStage > 0 then
                            ...
                            if progressPercent >= segmentStart and progressPercent <= segmentEnd then
                                local segmentDuration = segmentEnd - segmentStart
                                if segmentDuration > 0 then
                                    stageProgress = math_min(1.0, math_max(0, (progressPercent - segmentStart) / segmentDuration))
                                end
                        ...
                        if progressPercent >= holdAtMaxStartPercent then
                            ...
                            if progressPercent >= holdAtMaxStartPercent and progressPercent <= holdAtMaxEndPercent then
                                ...
                                if holdAtMaxDuration > 0 then
                                    holdAtMaxProgress = math_min(1.0, math_max(0, (progressPercent - holdAtMaxStartPercent) / holdAtMaxDuration))
                                end
                            ...
                            local fillWidth = math_max(0, holdAtMaxSegmentWidth * holdAtMaxProgress)
    ...
    if frame.timeText then
        ...
        if cfg and cfg.showTimeText ~= false then
            frame.timeText:Show()
            local format = cfg.timeTextFormat or "current/total"
            frame.timeText:SetText(FormatCastTime(progress, duration, format))
        else
            frame.timeText:Hide()
        end
    end
...
function CastBars:ShowTestCastBar()
    ...
    local cfg = DDingUI.db.profile.castBar
    local font = DDingUI:GetGlobalFont()
...
DDingUI.ShowTestCastBar = function(self) return CastBars:ShowTestCastBar() end
