...
function CastBars:GetCastBar()
    ...
    bar:SetIgnoreParentAlpha(true) -- [REFACTOR] ContainerSync가 뷰어 alpha=0으로 은닉해도 바 유지
    ...
    local initWidth
    local effectiveW, needsBorderComp = DDingUI:GetEffectiveAnchorWidth(anchor)
    if effectiveW and effectiveW > 0 and effectiveW < 1000 and anchor ~= UIParent then
        if needsBorderComp then
            local bComp = DDingUI:Scale(1)
            initWidth = PixelSnap(effectiveW - 2 * bComp)
        else
...
function CastBars:UpdateCastBarLayout()
    ...
    local anchorPoint = cfg.anchorPoint or "CENTER"
    local desiredX = DDingUI:Scale(cfg.offsetX or 0)
    ...
    if not (DDingUI.Movers and DDingUI.Movers.ConfigMode) then
        ...
        if width <= 0 then
            ...
            if effectiveW and effectiveW > 0 and effectiveW < 1000 and anchor ~= UIParent then
                if needsBorderComp then
                    local bComp = DDingUI:Scale(1)
                    width = PixelSnap(effectiveW - 2 * bComp)
                else
    ...
    bar.timeText:SetPoint("RIGHT", bar.status, "RIGHT", DDingUI:Scale(cfg.timeTextOffsetX or -4), DDingUI:Scale(cfg.timeTextOffsetY or 0))

    local spellFontName = cfg.spellTextFont or cfg.textFont or DDingUI.DEFAULT_FONT_NAME
    local spellFont = DDingUI:GetFont(spellFontName)
    local spellSize = cfg.spellTextSize or cfg.textSize or 10
    bar.spellName:SetFont(spellFont, spellSize, "OUTLINE")
    bar.spellName:SetShadowOffset(0, 0)

    if cfg.showSpellText ~= false then
        bar.spellName:Show()
    else
        bar.spellName:Hide()
    end

    local timeFontName = cfg.timeTextFont or cfg.textFont or DDingUI.DEFAULT_FONT_NAME
    local timeFont = DDingUI:GetFont(timeFontName)
    local timeSize = cfg.timeTextSize or cfg.textSize or 10
    bar.timeText:SetFont(timeFont, timeSize, "OUTLINE")
