...
-- [REFACTOR] Í≥µÌÜµ TextureBorder ?†Ìã∏Î¶¨Ìã∞ (Toolkit.lua) ??CustomIcons???àÏ™Ω Î≥¥Îçî(inset=true)
...
-- ?àÏ™Ω Î≥¥Îçî ?òÌçº (inset=true ?êÎèô ?ÑÎã¨)
...
-- [RACIALS] Ï¢ÖÏ°± ?πÏÑ± Îß§Ìïë (?êÎèô Í∞êÏ???
local RACIAL_SPELLS = {
...
-- [REFACTOR] CreateBackdrop?Ä GUI.luaÎ°??¥Îèô????EnsureGUILoaded()?êÏÑú lazy-load
...
-- Icon updates
-- ------------------------
-- [FIX] IsCooldownFrameActive: ?¥Ï†Ñ ?®Ïπò?êÏÑú ?ïÏùò ?úÍ±∞????forward declaration ?†Ï? (nil)
-- L830 ?∏Ï∂úÎ∂Ä??GetCooldownTimes Í∏∞Î∞ò?ºÎ°ú ÍµêÏ≤¥?òÏóà?ºÎ?Î°????¥ÏÉÅ Î≥¥Ïïà ?ÑÏöî ?ÜÏùå

-- [CDM CDM Î∞©Ïãù] ?ÑÏù¥?????§Ìé† Ïø®Îã§??Îß§Ìïë
-- ?ÑÏù¥??Ïø®Îã§??APIÍ∞Ä ?ÑÌà¨ Ï§???≤å Í∞±Ïã†?????§Ìé† Ïø®Îã§?¥ÏúºÎ°??¥Î∞±
...
-- [FIX CDM] IsCooldownFrameActive ?úÍ±∞ ???Ä??EvaluateRemainingDuration(GCDFilterCurve)Î°?frame-perfect Ïø®Îã§??Í∞êÏ?
local function EvalDesatFromDurObj(durObj, isOnGCD)
    ...
    -- fallback: curve ÎØ∏ÏÉù???òÍ≤Ω (WoW Íµ?Î≤ÑÏ†Ñ) ??durObjÍ∞Ä ?àÍ≥† GCDÍ∞Ä ?ÑÎãàÎ©?1
...
local function UpdateItemIcon(iconFrame, iconData)
    ...
    if itemSpellID then
        -- ?§Ìé† IDÍ∞Ä Îß§Ìïë???ÑÏù¥?? CDM??ÏµúÏö∞??ItemCD ?úÎèÑ, ?§Ìå®??SpellDur ?¨Ïö©
        ...
        -- [Fallback] ?§Ìé† ID ?ÜÎäî ?ÑÏù¥??(ÎπÑÏ†Ñ???úÌïú???ëÎèô)
        local itemCdStart, itemCdDuration, hasItemCooldown, itemCdSafe =
            ResolveItemCooldownSpan(iconFrame, "_ddItemCooldown", activeItemID, nil, itemSpellID)
    ...
    -- [FIX] OnUpdate ÏßÑÏûÖ Ï°∞Í±¥: cdInfo.isActive (safe boolean) ?¨Ïö© ??secret number ÎπÑÍµê Í∏àÏ?
    ...
    elseif allowCooldownDesat and desatDurationObject and desatSpellID then
        -- isOnRealCD: boolean (safe) ??secret number ÎπÑÍµê ?ÜÏùå
        pcall(function()
            local cdInfo = C_Spell.GetSpellCooldown(desatSpellID)
            if cdInfo and cdInfo.isActive and cdInfo.isOnGCD ~= true then
                itemIsOnRealCD = true
                -- isOnGCD???¥Î? false?¥Î?Î°?EvalDesatFromDurObj??false ?ÑÎã¨
            end
        end)
        if itemIsOnRealCD then
            -- EvaluateRemainingDuration Í≤∞Í≥º(secret)??ÎπÑÍµê ?ÜÏù¥ SetDesaturation??ÏßÅÏ†ë ?ÑÎã¨
    ...
    -- OnUpdate Î£®ÌîÑ: isOnRealCD (safe boolean)?ºÎ°úÎß?ÏßÑÏûÖ ?êÎã® ??desatVal ÎπÑÍµê Í∏àÏ?
    if itemCombatLocked then
...
local function UpdateSpellIconFrame(iconFrame, iconData)
    ...
    -- ?çÏä§Ï≤òÎèô??Í∞±Ïã† (?§Î≤Ñ?ºÏù¥???ÑÎùΩ Ï¥àÍ∏∞Î°úÎìú ?Ä??
    ...
    -- [CDM ?®ÌÑ¥] Ïø®Îã§??DurationObject ?†ÌÉù
    -- CCD(ChargeDuration)Í∞Ä Ï°¥Ïû¨?òÎ©¥ Ï∂©Ï†Ñ Ïø®Îã§?? ?ÜÏúºÎ©?SCD(SpellCooldown) ?¨Ïö©
    -- currentCharges/maxCharges Í∞?ÎπÑÍµê Î∂àÌïÑ??(secret value taint Î∞©Ï?)
    local durObj
    ...
    -- [FIX ÍπúÎπ°?? Ïø®Îã§???ÅÌÉú Ï∫êÏã± ???ôÏùº ?ÅÌÉú ?¨Set/Clear Î∞©Ï?
    -- GCD ÏßÑÏûÖ/?¥Ï†ú ??durObj Î≥Ä?îÍ? ?ÜÏúºÎ©?cooldown ?ÑÎ†à?ÑÏùÑ Í±¥ÎìúÎ¶¨Ï? ?äÏùå
    ...
    -- [FIX CDM] Swipe/Edge ?§Ì???(Î≥ÄÍ≤??ÜÏùå)
    ...
    -- [FIX CDM] EvaluateRemainingDuration Í∏∞Î∞ò ?àÏÉâ
    -- secret numberÎ•?ÎπÑÍµê?òÏ? ?äÏùå ??cdInfo.isActive (safe boolean)?ºÎ°ú OnUpdate ÏßÑÏûÖ Í≤∞Ï†ï
    ...
    if usable then
        if allowDesat then
            -- 1?®Í≥Ñ: safe boolean?ºÎ°ú Ïø®Îã§???úÏÑ± ?¨Î? ?êÎã®
            pcall(function()
                local cdInfo = C_Spell.GetSpellCooldown(spellID)
                if cdInfo and cdInfo.isActive and cdInfo.isOnGCD ~= true then
                    isOnRealCD = true
            ...
            -- 2?®Í≥Ñ: secret number??ÎπÑÍµê ?ÜÏù¥ SetDesaturation?êÎßå ?ÑÎã¨
            -- [FIX ÍπúÎπ°?? GCD ÏßÑÏûÖ ???àÏÉâÍ∞?Î≥¥Ï°¥ ??isOnGCDÎ©??¥Ï†Ñ Í∞??†Ï?, ?ÜÏúºÎ©?0
            if desatDurObj then
                if isOnGCD then
                    -- GCD Ï§? ÏßÅÏ†Ñ ?àÏÉâÍ∞íÏùÑ Í∑∏Î?Î°??†Ï? (?∞ÏÉâ?îÌöå??ÍµêÎ≤à Î∞©Ï?)
        ...
        iconFrame._lastDesatValue = nil  -- unusable ?ÅÌÉú ?ÑÌôò ??Ï∫êÏãú Ï¥àÍ∏∞??    end
    iconFrame.icon:SetDesaturation(desatValue)

    -- OnUpdate Î£®ÌîÑ: isOnRealCD (safe boolean)ÎßåÏúºÎ°?ÏßÑÏûÖ Í≤∞Ï†ï ??desatValue ÎπÑÍµê Í∏àÏ?
...
local function UpdateTrinketProcIcon(iconFrame, iconData)
    ...
    if auraData then
        ...
        -- [FIX] Í∏ÄÎ°úÏö∞Î•??ÑÏù¥ÏΩ??∏Î?Î°??ïÏû•?òÏó¨ ?§Ï??¥ÌîÑ?Ä Í≤πÏπòÏßÄ ?äÎäî ?åÎëêÎ¶¨Î°ú ?¨Ïö©
        -- ?§Ï??¥ÌîÑ???ÑÏù¥ÏΩ??¥Î??êÎßå Í∑∏Î†§ÏßÄÎØÄÎ°? ?∏Í≥Ω?ºÎ°ú Î≤óÏñ¥??Í∏ÄÎ°úÏö∞????ÉÅ Î≥¥ÏûÑ
        -- xOffset/yOffset?ºÎ°ú ?ÑÏù¥ÏΩ?Í≤ΩÍ≥Ñ Î∞?8px Ï∂îÍ? ?ïÏû•
        local LCG = LibStub("LibCustomGlow-1.0", true)
...
local function ApplyIconSettings(iconFrame, iconData, groupSettings)
    ...
    -- [FIX] DynamicIconBridge Í¥ÄÎ¶??ÑÏù¥ÏΩòÏ? GroupSystem???¨Í∏∞Î•?Í¥ÄÎ¶¨ÌïòÎØÄÎ°?Í±¥ÎÑà?Ä
    -- CustomIcons??aspectRatio?Ä GroupSystem??aspectRatioCrop???§Î•¥Î©?    -- SetSize ??snap-back ????1?ÑÎ†à??ÍπúÎπ°??Î∞úÏÉù Î∞©Ï?
...
-- ------------------------
-- Aura (buff/debuff) icon update ??trinketProc ?®ÌÑ¥ Í∏∞Î∞ò
-- CDM reparentÍ∞Ä ?ÑÎãå ?ÖÎ¶Ω ?ÑÎ†à?ÑÏúºÎ°?buff Ï∂îÏ†Å
-- ------------------------
...
local function UpdateAuraIcon(iconFrame, iconData)
    ...
    -- spellName Í∏∞Î∞ò ?¥Î∞± (buff spellID ??spell spellID??Í≤ΩÏö∞)
    ...
    -- Ï∫êÏãú??buff spellIDÎ°??¨Ïãú??    if not timedOnly and not auraData and iconFrame._cachedAuraSpellID then
        pcall(function()
            if not auraData then
    ...
    if auraData then
        -- [FIX] Î≤ÑÌîÑ ?§Ï??¥ÌîÑ Î∞©Ìñ•: fill-up (CDM ?®ÌÑ¥)
        iconFrame.cooldown:SetReverse(true)

        -- ?úÏÑ±: duration Ïø®Îã§??+ ?§ÌÉù ?úÏãú
        pcall(function()
            local auraDuration = GetAuraNumberFieldSafe(auraData, "duration")
            local auraExpiration = GetAuraNumberFieldSafe(auraData, "expirationTime")
            if auraDuration and auraDuration > 0 and auraExpiration then
        ...
        -- [FIX] managed ?ÑÎ†à?ÑÏ? GroupRendererÍ∞Ä Show/Hide Í¥ÄÎ¶?
        if not iconFrame._ddIsManaged then
            iconFrame:Hide()
        end
    end
...
-- [CDM ?®ÌÑ¥] ?îÎ∞î?¥Ïä§ ?ÅÌÉú ??Í∞ôÏ? ?±Ïóê ?¨Îü¨ ?¥Î≤§?∏Í? ?ôÏãú??UpdateAllIconsÎ•?-- ?∏Ï∂ú?¥ÎèÑ ?§Ï†ú ?§Ìñâ?Ä ?§Ïùå ?ÑÎ†à?ÑÏóê ??1?åÎßå ?òÌñâ (C_Timer.After(0) Î∞∞Ïπò Ï≤òÎ¶¨)
...
-- [CDM ?®ÌÑ¥] Í≥µÍ∞ú ÏßÑÏûÖ?????¥Î≤§???∏Îì§?¨Îäî ???®ÏàòÎß??∏Ï∂ú
-- Í∞ôÏ? ?????§Ïàò ?∏Ï∂ú??1?åÎ°ú Î≥ëÌï©, ?§Ïùå ?ÑÎ†à?ÑÏóê ?§Ìñâ
UpdateAllIcons = function(needsLayoutNotify, filter)
    ...
    C_Timer.After(delay, function()
        if capturedSeq ~= _iconUpdateSeq then return end  -- ??ÏµúÏã† ?îÏ≤≠???àÏúºÎ©??§ÌÇµ
...
local function ScheduleSpecReload()
    ...
    -- [FIX] ?§Îã®Í≥??¨Ïãú?? CDM Î∑∞Ïñ¥ ?¨ÏÉù???ÄÍ∏?    -- Phase 1 (0.3s): Îπ†Î•∏ Ï¥àÍ∏∞ Í∞±Ïã†
    C_Timer.After(0.3, function()
        runtime.pendingSpecReload = false
        -- trinket proc Ï∫êÏãú Î¨¥Ìö®??
    ...
    -- Phase 2 (1.5s): CDM ?àÏ†ï????ÏµúÏ¢Ö Í∞±Ïã†
...
local function CreateBaseIcon(name, parent)
    ...
    -- [FIX] ARTWORK ?àÏù¥???¨Ïö©: BackdropTemplate??backdrop??BACKGROUND ?àÏù¥?¥Î? Ï∞®Ï??òÎ?Î°?    -- BACKGROUND??icon??ÎßåÎì§Î©?backdrop??Í∞Ä?§Ï†∏ ?¨Î™Ö?òÍ≤å Î≥¥ÏûÑ
...
local function CreateItemIcon(iconKey, iconData, parent)
    ...
    -- [FIX] CDM Î∞©Ïãù: ?ÑÎ†à?ÑÏ? ??ÉÅ ?ùÏÑ±, ?çÏä§Ï≤òÎßå ?òÏ§ë???ÖÎç∞?¥Ìä∏
    -- GetItemInfoÍ∞Ä nil?¥Ïñ¥???ÑÎ†à?ÑÏ? ÎßåÎì§?¥Ïïº GroupSystem??Ï∂îÏ†Å Í∞Ä??    local itemName = GetItemInfo(itemID)
...
local function CreateSpellIcon(iconKey, iconData, parent)
    local spellID = iconData.id
    if not spellID then return nil end
    -- [FIX] ?§Ìé†Î∂?Ï≤¥ÌÅ¨??IsIconActive?êÏÑú ?òÎ?Î°??¨Í∏∞?úÎäî ?ÑÎ†à?ÑÎßå ?ùÏÑ±
    -- ?†Ï?Í∞Ä Ï∂îÍ????§Ìé†???ÑÏû¨ ?πÏÑ±???ÜÎçî?ºÎèÑ ?ÑÎ†à?ÑÏ? Ï°¥Ïû¨?¥Ïïº ??
...
local function CreateSlotIcon(iconKey, iconData, parent)
    ...
    -- [FIX] ?çÏä§Ï≤???ÉÅ ?§Ï†ï ??GetItemInfo Ï∫êÏãú ÎØ∏Ïä§ ?úÏóê???ÑÏù¥ÏΩ?Î≥¥Ïù¥?ÑÎ°ù
    local itemID = CustomIcons.GetEquippedSlotItemID(frame, slotID)
    local tex = nil
    if itemID then
        -- GetItemInfoÎ≥¥Îã§ GetInventoryItemTextureÍ∞Ä ???†Î¢∞?????àÏùå (Ï∫êÏãú Î∂àÌïÑ??
        tex = GetInventoryItemTexture("player", slotID)
        if not tex then
            local _, _, _, _, _, _, _, _, _, itemTexture = GetItemInfo(itemID)
            tex = itemTexture
...
local function CreateAuraIcon(iconKey, iconData, parent)
    ...
    -- ?çÏä§Ï≤? C_Spell.GetSpellTexture ??GetSpellInfo.iconID ?¥Î∞±
    SetStableIconTexture(frame, ResolveSpellTexture(spellID, frame._fallbackTexture), true)
    return frame
end
...
local function CreateDynamicIcon(iconKey, iconData, parent)
    ...
    elseif iconData.type == "racial" then
        ...
        -- ?ÑÏãú ?åÏù¥Î∏îÎ°ú CreateSpellIcon???∏Ï∂ú?òÏó¨ ?∞Ïù¥???§Ïóº Î∞©Ï?
...
function Layout.LayoutGroup(groupKey, iconKeys)
    -- [DYNAMIC] GroupSystem???úÏÑ±?¥Î©¥ ?àÏù¥?ÑÏõÉ ?§ÌÇµ (GroupRendererÍ∞Ä ?Ä??Ï≤òÎ¶¨)
    if DDingUI.DynamicIconBridge and DDingUI.DynamicIconBridge:IsActive() then
        return
    end
    local db = GetDynamicDB()
...
local function RefreshAllLayouts()
    ...
    -- SpecProfiles ?êÎèô ?Ä???∏Î¶¨Í±?(?ôÏ†Å ?ÑÏù¥ÏΩ??§Ï†ï Î≥ÄÍ≤?Í∞êÏ?)
    if DDingUI.SpecProfiles and DDingUI.SpecProfiles.MarkDirty then
        DDingUI.SpecProfiles:MarkDirty()
    end
    if runtime.RegisterCustomCooldownWatches then
...
function CustomIcons:LoadDynamicIcons()
    ...
    -- ?ÑÎ°ú??Î≥ÄÍ≤???Í∏∞Ï°¥ ?ÑÎ†à???ïÎ¶¨: db???ÜÎäî ?ÑÏù¥ÏΩ??úÍ±∞
    for iconKey, frame in pairs(runtime.iconFrames) do
    ...
    -- [FIX] ?ÑÎ†à???ùÏÑ± ?§Ìå®???ÑÏù¥ÏΩ??òÏßë (?ÑÏù¥??Ï∫êÏãú ÎØ∏Ï?Îπ???
    local pendingKeys = {}
    ...
    for iconKey, iconData in pairs(db.iconData) do
        ...
        -- [FIX] Î°úÍ∑∏??ÏßÅÌõÑ(10Ï¥??¥ÎÇ¥) ?§Ìé†Î∂ÅÏù¥ Ï§ÄÎπ????òÏñ¥ falseÎ•?Î∞òÌôò?òÎäî Í≤ΩÏö∞ ?§Ìå®Î°?Í∞ÑÏ£º?òÏ? ?äÍ≥† ?¨Ïãú???ÄÍ∏∞Ïó¥??Ï∂îÍ?
    ...
    -- [FIX] GroupSystem???úÏÑ±?¥Î©¥ CustomIcons ?êÏ≤¥ ?ÑÎ†à?ÑÏùÑ Ï¶âÏãú ?®Í?
    -- CreateDynamicIcon??Show()Î•??∏Ï∂ú?òÏó¨ Î¶¨Î°ú?????åÏÉâ ?ÑÎ†à?ÑÏù¥ ?†Íπê Î≥¥Ïù¥??Í≤?Î∞©Ï?
    -- GroupSystem???êÏ≤¥ ?àÏù¥?ÑÏõÉ?ºÎ°ú Í¥ÄÎ¶¨ÌïòÎØÄÎ°?CustomIcons ?ÑÎ†à?ÑÏ? Î≥¥Ïùº ?ÑÏöî ?ÜÏùå
    local bridge = DDingUI.DynamicIconBridge
    ...
    -- [FIX] ?ÑÎ†à???ùÏÑ± ?§Ìå®???ÑÏù¥ÏΩ??¨Ïãú??(?ÑÏù¥???§Ìé† Ï∫êÏãú Î°úÎìú ?ÄÍ∏?
    if #pendingKeys > 0 then
        ...
        retryTimer = C_Timer.NewTicker(1.0, function()
            ...
            if #pendingKeys == 0 or attempts >= maxAttempts then
                ...
                -- ?àÎ°ú ?ùÏÑ±???ÑÎ†à?ÑÏù¥ ?àÏúºÎ©??àÏù¥?ÑÏõÉ Í∞±Ïã† + GroupSystem ?åÎ¶º
                RefreshAllLayouts()
                UpdateAllIcons(nil, "all")
            end
        end)
...
-- [FIX] FlightHide?êÏÑú ?§Ïù¥?òÎ? Í∑∏Î£π ?ÑÎ†à???åÌåå ?ÅÏö©???ÑÌïú getter
...
function CustomIcons:AddDynamicIcon(iconData)
    ...
    -- Build frame ??CreateDynamicIcon?Ä ??ÉÅ ?ÑÎ†à??Î∞òÌôò (CDM Î∞©Ïãù)
    ...
    -- [FIX] GroupSystem Ï¶âÏãú Í∞±Ïã† ???îÎ∞î?¥Ïä§ ?∞Ìöå?òÏó¨ ?ÑÏù¥ÏΩ?Î∞îÎ°ú ?úÏãú
    ...
    -- [FIX] SpecProfiles Ï¶âÏãú ?Ä????Î¶¨Î°ú????LoadSpec???¥Ï†Ñ ?§ÎÉÖ?∑ÏúºÎ°?Î≥µÏõê?òÏó¨ ?∞Ïù¥???êÏã§ Î∞©Ï?
...
function CustomIcons:RemoveGroup(groupKey)
    ...
    -- Í∑∏Î£π ???ÑÏù¥ÏΩòÎì§???§Ï†úÎ°???†ú
    local iconsToRemove = {}
    for _, iconKey in ipairs(group.icons or {}) do
        iconsToRemove[#iconsToRemove + 1] = iconKey
    end
    -- [FIX] DynamicIconBridge managed ?ÅÌÉú???ïÎ¶¨ (DestroyGroup??ReleaseFrame??Î≥µÏõê?òÎäî Í≤?Î∞©Ï?)
    local bridge = DDingUI.DynamicIconBridge
    for _, iconKey in ipairs(iconsToRemove) do
        ...
        -- bridge managed ?ÅÌÉú ?ïÎ¶¨ (DestroyGroup?íReleaseFrame??orig.parentÎ°?Î≥µÏõê?òÎäî Í≤?Î∞©Ï?)
        local frame = runtime.iconFrames[iconKey]
        if frame then
    ...
    -- [FIX] Í∑∏Î£π Ïª®ÌÖå?¥ÎÑà ?ÑÎ†à?ÑÎèÑ ?ïÎ¶¨ (Í≥†Ïä§??Ïª®ÌÖå?¥ÎÑà Î∞©Ï?)
...
function CustomIcons:MoveIconToGroup(iconKey, targetGroup)
    ...
    RefreshAllLayouts()
    -- [FIX] GroupSystem Ï¶âÏãú Í∞±Ïã† ??MoveIconToGroup ??Î∞îÎ°ú ?ÑÏù¥ÏΩ??úÏãú
    C_Timer.After(0.1, function()
        local bridge = DDingUI.DynamicIconBridge
        if bridge and bridge:IsActive() then
            local gs = DDingUI.GroupSystem
    ...
    CustomIcons:RefreshDynamicListUI()
    -- [FIX] SpecProfiles Ï¶âÏãú ?Ä????MoveIconToGroup ???§ÎÉÖ??Í∞±Ïã†
    if DDingUI.SpecProfiles and DDingUI.SpecProfiles.SaveCurrentSpec then
        DDingUI.SpecProfiles:SaveCurrentSpec()
    end
    -- [FIX] GroupSystem ?µÏÖò ?∏Î¶¨ ?¨Îπå?????†Îãπ Î™©Î°ù?????ÑÏù¥ÏΩ?Ï¶âÏãú ?úÏãú
...
-- [FIX] Î∞©Ìñ• ?¥Îèô (?ëÏúÑ/?ìÏïÑ??: Í∑∏Î£π ???ÑÏù¥ÏΩ??úÏÑú Î≥ÄÍ≤?
...
local function CreateIconNode(parent, iconKey, iconData, groupKey)
    ...
    -- [STYLE] bg.input Í∏∞Î≥∏, bg.hover ?∏Î≤Ñ, bg.selected ?†ÌÉù
    ...
    -- Multi-select checkbox (UF ?µÏùº: 14x14, Í∑∏Îùº?îÏñ∏??Ï≤¥ÌÅ¨)
    ...
    -- Ï≤¥ÌÅ¨ ÎßàÌÅ¨ ?çÏä§Ï≥?(UF ?µÏùº: ?ÑÏ≤¥ Ï±ÑÏö∞Í∏?Í∑∏Îùº?îÏñ∏??
    ...
    -- ?òÏù¥?ºÏù¥??    local cbHighlight = checkbox:CreateTexture(nil, "ARTWORK")
...
-- [REFACTOR] GUIRefs.CreateStyledButton/Toggle/Input/Dropdown ??DDingUI.GUIÎ°??¥Îèô (EnsureGUILoaded?êÏÑú Î°úÎìú)
...
function CustomIcons:RefreshDynamicListUI()
    ...
    local selectAllBtn = GUIRefs.CreateStyledButton(selectBtnFrame, "?ÑÏ≤¥ ?†ÌÉù", 75, 22)
    ...
    local deselectAllBtn = GUIRefs.CreateStyledButton(selectBtnFrame, "?†ÌÉù ?¥Ï†ú", 75, 22)
    ...
    if selectedCount > 0 then
        countText:SetText("|cff00ff00" .. selectedCount .. "Í∞??†ÌÉù??r")
    else
        countText:SetText("")
    end
    ...
    local function renderSection(title, iconKeys, groupKey)
        ...
        -- [STYLE] Í∑∏Î£π ?§Îçî bg.widget, border.default
    ...
    -- ?êÏãù ?ÑÏ†Ø ?ÑÏóê?úÎèÑ ?§ÌÅ¨Î°?Í∞Ä?•Ìïò?ÑÎ°ù ÎßàÏö∞?????ÑÌåå
...
function CustomIcons:RefreshDynamicConfigUI()
    ...
    if selectedCount > 1 then
        ...
        header:SetText(selectedCount .. "Í∞??ÑÏù¥ÏΩ??ºÍ¥Ñ ?∏Ïßë")
        y = y + 30
    ...
    local function showIconConfig()
        ...
        if iconData.type == "trinketProc" then
            ...
            procDesc:SetText("0: Use: ?®Í≥º ?êÎèô Í∞êÏ? / ?òÎèô: ?®ÏãúÎ∏??ÑÎ°ù spellID ?ÖÎ†•")
        ...
        if isItemType then
            ...
            fallbackDesc:SetText("?? 3??Î¨ºÏïΩID, 2?±ID, 1?±ID (?ºÌëú Íµ¨Î∂Ñ)")
...
function CustomIcons:BuildDynamicIconsUI(parent)
    ...
    -- ?¥Ï†Ñ uiFrames Ï∞∏Ï°∞ Ï¥àÍ∏∞??(?¨ÏßÑ?????îÏÉÅ Î∞©Ï?)
    ...
    -- [STYLE] ?ÖÏÑº???çÏä§??    if createIconBtn.text then createIconBtn.text:SetTextColor(GUIRefs.THEME.accent[1], GUIRefs.THEME.accent[2], GUIRefs.THEME.accent[3], 1) end
    if search.editBox then
    ...
    -- [STYLE] ?ÖÏÑº???çÏä§??    if createGroupBtn.text then createGroupBtn.text:SetTextColor(GUIRefs.THEME.accent[1], GUIRefs.THEME.accent[2], GUIRefs.THEME.accent[3], 1) end
    ...
    -- [STYLE] Ï¢åÏö∞ Íµ¨Î∂Ñ??(border.separator)
    ...
    -- [STYLE] ?∞Ï∏° ?§Ï†ï ?ÅÏó≠: bg.sidebar Î∞∞Í≤Ω, border.default ?åÎëêÎ¶?    local configContainer = CreateFrame("Frame", nil, container, "BackdropTemplate")
...
-- Î™®Îì† ?úÏÑ± ?ÑÏù¥ÏΩ??ÑÎ†à??Î∞òÌôò (runtime.iconFrames Ï∞∏Ï°∞)
function CustomIcons:GetAllIconFrames()
    return runtime.iconFrames
end

-- [INTEGRATION] GroupRendererÍ∞Ä ÏßÅÏ†ë ?∏Ï∂ú?????àÎèÑÎ°?export
