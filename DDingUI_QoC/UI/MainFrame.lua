...
local function CreateMainFrame()
    local frame = UI:CreateMainFrame(UIParent, 800, 600, "DDingQoC_MainFrame")

    -- 타이틀바 (버전 표시)
    local titleBar = UI:CreateTitleBar(frame, "|cffffffffDDing|r|cffffa300UI|r |cffd93380QoC|r v" .. DDingQoC.version)
    ...
    -- ESC로 닫기
    tinsert(UISpecialFrames, "DDingQoC_MainFrame")
...
local function CreateGeneralPanel(parent)
    ...
    for i, mod in ipairs(modules) do
        ...
        local checkbox = UI:CreateCheckbox(panel, mod.display, function(checked)
            if checked then
                DDingQoC:EnableModule(mod.name)
            else
                DDingQoC:DisableModule(mod.name)
            end
        end)
    ...
    local minimapCheckbox = UI:CreateCheckbox(panel, L["SHOW_MINIMAP_BUTTON"], function(checked)
        ...
        if DDingQoC.UpdateMinimapButton then
            DDingQoC:UpdateMinimapButton()
        end
    end)
    ...
    versionText:SetText(L["VERSION"] .. ": " .. DDingQoC.version)
...
local function Initialize()
    ...
    mainFrame:AddTab("CombatTimer", L["TAB_COMBATTIMER"], function(parent)
        local module = DDingQoC:GetModule("CombatTimer")
