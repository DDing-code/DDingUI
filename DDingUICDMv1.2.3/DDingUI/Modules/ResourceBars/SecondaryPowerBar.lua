...
function ResourceBars:GetSecondaryPowerBar()
    ...
    bar:SetPoint("CENTER", anchor, anchorPoint, DDingUI:Scale(cfg.offsetX or 0), DDingUI:Scale(cfg.offsetY or 12))

    local MAX_AUTO_WIDTH_INIT = 600
    local width = cfg.width or 0
    if width <= 0 then
        -- Auto width: get from viewer or anchor content width
        local effectiveWidth = DDingUI:GetEffectiveAnchorWidth(anchor)
...
function ResourceBars:UpdatePerPointColorSegments(bar, resource, visualMax, current)
    ...
    if resource == Enum.PowerType.SoulShards then
        displayMax = math.floor(visualMax / 10)
    ...
    displayCurrent = math.min(displayCurrent, displayMax)
...
function ResourceBars:UpdateSecondaryPowerBar()
    ...
    do -- positioning block
        ...
        if (bar._lastAnchor ~= anchor or bar._lastAnchorPoint ~= anchorPoint or bar._lastOffsetX ~= desiredX or bar._lastOffsetY ~= desiredY) and not isInMoverMode then
            bar:ClearAllPoints()
    ...
    if fragmentedPowerTypes[resource] then
        ...
        bar.StatusBar:SetValue(math.min(current, visualMax), interpolation)
...
function ResourceBars:UpdateSecondaryPowerBarMarkers(bar, max)
    ...
    for i, rawVal in ipairs(markers) do
        ...
        if val > 0 and val < max then
            local x = math.floor((val / max) * width)
