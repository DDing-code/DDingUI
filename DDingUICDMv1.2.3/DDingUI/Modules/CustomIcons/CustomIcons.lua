...
-- Helper function to create a styled button for dialogs
local function CreateStyledButton(parent, text, width, height)
    if not EnsureGUILoaded() then return CreateFrame("Button", nil, parent) end
    ...
    btn:SetBackdropColor(THEME.bgWidget[1], THEME.bgWidget[2], THEME.bgWidget[3], THEME.bgWidget[4] or 0.80)
    btn:SetBackdropBorderColor(THEME.border[1], THEME.border[2], THEME.border[3], THEME.border[4] or 0.50)

    local globalFont = DDingUI:GetGlobalFont() or "Fonts\\2002.TTF"
    btn.text = btn:CreateFontString(nil, "OVERLAY")
    btn.text:SetFont(globalFont, 11, "")
    ...
    btn:SetScript("OnLeave", function(self)
        self:SetBackdropColor(THEME.bgWidget[1], THEME.bgWidget[2], THEME.bgWidget[3], THEME.bgWidget[4] or 0.80)
        self:SetBackdropBorderColor(THEME.border[1], THEME.border[2], THEME.border[3], THEME.border[4] or 0.50)
    end)
...
-- Helper function to create a styled toggle button (radio-like)
local function CreateStyledToggle(parent, text, width)
    ...
    btn.text:SetPoint("CENTER")
    btn.text:SetText(text)
    btn.text:SetTextColor(THEME.textDim[1], THEME.textDim[2], THEME.textDim[3], 1)
...
-- Helper function to create a styled input box
local function CreateStyledInput(parent, width, height, numeric)
    if not EnsureGUILoaded() then return CreateFrame("Frame", nil, parent) end
    local globalFont = DDingUI:GetGlobalFont() or "Fonts\\2002.TTF"
    ...
    container:SetBackdropColor(THEME.bgWidget[1], THEME.bgWidget[2], THEME.bgWidget[3], THEME.bgWidget[4] or 0.80)
    container:SetBackdropBorderColor(THEME.border[1], THEME.border[2], THEME.border[3], THEME.border[4] or 0.50)

    local editBox = CreateFrame("EditBox", nil, container)
    editBox:SetPoint("TOPLEFT", 6, -4)
    editBox:SetPoint("BOTTOMRIGHT", -6, 4)
    editBox:SetAutoFocus(false)
    editBox:SetFont(globalFont, 11, "")
    ...
    container.SetText = function(self, text) self.editBox:SetText(text) end

    return container
end

-- Helper function to create a styled dropdown
local function CreateStyledDropdown(parent, options, width)
    ...
    container.text:SetFont(globalFont, 11, "")
    ...
    container.text:SetTextColor(THEME.text[1], THEME.text[2], THEME.text[3], 1)

    local arrow = container:CreateFontString(nil, "OVERLAY")
    arrow:SetFont(globalFont, 10, "")
    ...
    menu:SetBackdropBorderColor(THEME.border[1], THEME.border[2], THEME.border[3], THEME.border[4] or 0.50)
    ...
    container.SetText = function(self, text) self.text:SetText(text) end
    ...
    if selectedCount > 1 then
        ...
        Widgets.CreateExecute(uiFrames.configParent, {
            ...
            func = function()
                ...
                print("|cff00ff00[DDingUI] " .. selectedCount .. "개 아이콘에 설정이 적용되었습니다.|r")
            end,
            width = "full",
        }, y)
        y = y + 50
        ...
        Widgets.CreateExecute(uiFrames.configParent, {
            ...
            func = function()
                if selectedGroup and selectedGroup.icons then
                    ...
                    RefreshAllLayouts()
                    CustomIcons:RefreshDynamicListUI()
                    print("|cff00ff00[DDingUI] Applied group size to all icons|r")
                end
            end,
            width = "full",
        }, y)
...
function CustomIcons:BuildDynamicIconsUI(parent)
    ...
    if not EnsureGUILoaded() then
        print("|cffff0000[DDingUI]|r Dynamic Icons: GUI not loaded yet")
        return
    end
