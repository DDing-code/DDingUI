...
function IconViewers:SkinIcon(icon, settings)
    ...
    -- Skip placeholder icons (no layoutIndex = empty CDM slot) -- [FIX: placeholder black box]
    local isPlaceholder = false
    pcall(function() isPlaceholder = (icon.layoutIndex == nil and icon.cooldownID == nil) end)
    ...
    if icon.Cooldown then
                            ...
                            if LCG then
                                ...
                                local glowSuccess, err = pcall(function()
                                    if glowType == "Pixel Glow" then
                                        ...
                                        LCG.PixelGlow_Start(glowTarget, glowColor, pixelLines, pixelFrequency, pixelLength, pixelThickness, 0, 0, false, glowKey)
                                    elseif glowType == "Autocast Shine" then
                                        ...
                                        LCG.AutoCastGlow_Start(glowTarget, glowColor, particles, freq, scale, 0, 0, glowKey)
                                    elseif glowType == "Action Button Glow" then
                                        local freq = s.auraGlowButtonFrequency or 0.25
                                        LCG.ButtonGlow_Start(glowTarget, glowColor, freq)
                            ...
                            if LCG then
                                ...
                                pcall(function()
                                    if activeGlowType == "Pixel Glow" then
                                        LCG.PixelGlow_Stop(glowTarget, glowKey)
                                    elseif activeGlowType == "Autocast Shine" then
                                        LCG.AutoCastGlow_Stop(glowTarget, glowKey)
                                    elseif activeGlowType == "Action Button Glow" then
                                        LCG.ButtonGlow_Stop(glowTarget)
                                    elseif activeGlowType == "Proc Glow" then
                                        LCG.ProcGlow_Stop(glowTarget, glowKey)
        ...
        if settings.durationTextAnchor then
            ...
            local durationOffsetX = settings.durationTextOffsetX or 0
            local durationOffsetY = settings.durationTextOffsetY or 0
...
function IconViewers:SkinAllIconsInViewer(viewer)
    ...
    for _, icon in ipairs(children) do
        if IsCooldownIconFrame(icon) and (icon.icon or icon.Icon) then
            -- BuffIconCooldownViewer: skip placeholder icons (no layoutIndex = empty slot) -- [FIX: placeholder black box]
            local skipIcon = false
            if isBuffViewer then
                local isPlaceholder = false
                pcall(function() isPlaceholder = (icon.layoutIndex == nil) end)
                if isPlaceholder then
                    -- Hide any existing borders on placeholder to prevent black empty boxes
                    local id = iconData[icon]
                    if id and id.borders then
                        for _, borderTex in ipairs(id.borders) do
                            borderTex:SetShown(false)
            ...
            if not skipIcon then
                ...
                if not ok then
                    ...
                    print("|cffff4444[DDingUI] SkinIcon error for", name, "icon:", err, "|r")
                end
            end
        end
    end
