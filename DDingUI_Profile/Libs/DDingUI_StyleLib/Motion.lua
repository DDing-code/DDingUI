...
------------------------------------------------------
-- Colour and Ellesmere-style motion presets
------------------------------------------------------
...
--- Ellesmere-like button hover: short in/out colour interpolation for bg, border, and label.
function M.EllesmereButton(frame, opts)
    if not frame then return nil end
    opts = opts or {}
...
--- Red-to-normal validation flash, modelled after Ellesmere popup input feedback.
...
function M.EllesmereOpen(frame, opts)
    if not frame or not frame.SetAlpha then return nil end
    opts = opts or {}
    M.Stop(frame, opts.closeKey or "ellesmereClose")
...
function M.EllesmereClose(frame, opts)
    if not frame or not frame.SetAlpha then return nil end
    opts = opts or {}
    M.Stop(frame, opts.openKey or "ellesmereOpen")
    local base = opts.baseScale or (frame.GetScale and frame:GetScale()) or 1
    ...
    return M.Tween(frame, 0, 1, opts.duration or 0.14, {
        key = opts.key or "ellesmereClose",
...
Lib.EllesmereButtonMotion = M.EllesmereButton
Lib.EllesmereOpen = M.EllesmereOpen
Lib.EllesmereClose = M.EllesmereClose
