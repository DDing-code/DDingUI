...
-- CreateButton
------------------------------------------------------
--- Standard button with smooth Ellesmere-style hover & click flash.
--- @param parent Frame
--- @param addonName string
--- @param text string
...
function Lib.CreateButton(parent, addonName, text, onClick, opts)
    ...
    -- Ellesmere-style hover: short colour interpolation instead of a hard snap.
    local hoverMotion
    if Motion and opts.hoverMotion ~= false then
        hoverMotion = Motion.EllesmereButton(btn, {
...
function Lib.CreateSettingsPanel(addonName, title, version, opts)
    ...
    function frame:ShowAnimated(motionOpts)
        if Motion then
            Motion.EllesmereOpen(self, MotionOpts(motionOpts))
        else
            self:Show()
        end
    end
    ...
    function frame:HideAnimated(motionOpts)
        if Motion then
            Motion.EllesmereClose(self, MotionOpts(motionOpts))
        else
            self:Hide()
        end
    end
